...
//        }
    ...
    template<int dim,int degree, template<typename T> class MappingType>
    const std::vector<Eigen::Matrix<double,dim+1,degree+1>,Eigen::aligned_allocator<Eigen::Matrix<double,dim+1,degree+1>> >  LagrangeElement<dim,degree,MappingType>::sfCoeffsVector=LagrangeElement<dim,degree,MappingType>::get_sfCoeffsVector();
